...
{
    var tt1 = mod(t1, TWO_PI);
    var tt2 = mod(t2, TWO_PI);
...
function drawPhoton(x, y, cx, nano, t)
{
    ...
    rotate(t);
    {
        noFill();
        beginShape();
        var from = cx - width*0.04;
        var to = cx + width*0.04;
        for (var i = from; i < to; i += 0.5)
        {
            var py = width*0.02*pow(cos(map(i, from, to, -HALF_PI*2/3, HALF_PI*2/3)), 2) * sin(i*200000/(width*nano));
            curveVertex(i, py);
        }
        endShape(OPEN);
    }
...
{
    var c = color(getRedColor(nano), getGreenColor(nano), getBlueColor(nano));
    return c;
}
...
function drawArrow(x, y, cx, cy)
{
    ...
    // í™”ì‚´í‘œ ëª¨ì–‘
    var leng = sqrt(cx*cx + cy*cy);
    var bolt = min(leng*0.5, width*0.01);
    var x1 =  0.968912422 * cx*bolt/leng - 0.247403959 * cy*bolt/leng;
    var y1 =  0.247403959 * cx*bolt/leng + 0.968912422 * cy*bolt/leng;
    var x2 =  0.968912422 * cx*bolt/leng + 0.247403959 * cy*bolt/leng;
    var y2 = -0.247403959 * cx*bolt/leng + 0.968912422 * cy*bolt/leng;
    noFill();
    beginShape();
...
{
    var d = dist(x1, y1, x2, y2);
    var t1 = atan2(y2 - y1, x2 - x1);
    var xx1 = x1 + r1*cos(t1);
    var yy1 = y1 + r1*sin(t1);
    var xx2 = (x1 + x2) - r2*cos(t1);
    var yy2 = (y1 + y2) - r2*sin(t1);
    var xx = (d - r1 - r2)*cos(t1);
    var yy = (d - r1 - r2)*sin(t1);
...
{
    var lArrow = mag(cx, cy);	// ì „ì²´ ê¸¸ì´
    var wArrow = width*0.005;	// ë°˜ìª½ ë„“ì´
    var head = min(lArrow, 3*wArrow);
...
{
    // í™”ì‚´í‘œì˜ ê¸¸ì´
    var lArrow = round(sqrt(cx * cx + cy * cy));
