...
function mainApp(args) {
  ...
  function onload() {
      ...
      groundBodyDef.type = box2d.b2Body.b2_staticBody;
      ...
      particleSystem = world.GetParticleSystemList();
...

      {
          ...
          const vertices = [new box2d.b2Vec2(-4, -1), new box2d.b2Vec2(4, -1), new box2d.b2Vec2(4, 0), new box2d.b2Vec2(-4, 0)];
          shape.Set(vertices, 4);
          f=ground.CreateFixture( shape , 1.0);
          f.userData = {type: "square"}
      }

      {
          const shape = new box2d.b2PolygonShape();
          const vertices = [new box2d.b2Vec2(-4, -0.1), new box2d.b2Vec2(-2, -0.1), new box2d.b2Vec2(-2, 2), new box2d.b2Vec2(-4, 2)];
          shape.Set(vertices, 4);
          f=ground.CreateFixture( shape , 1.0);
          f.userData = {type: "square"}
      }
      ...
      {
          const shape = new box2d.b2PolygonShape();
          const vertices = [new box2d.b2Vec2(2, -0.1), new box2d.b2Vec2(4, -0.1), new box2d.b2Vec2(4, 2), new box2d.b2Vec2(2, 2)];
          shape.Set(vertices, 4);
          f=ground.CreateFixture( shape , 1.0);
          f.userData = {type: "square"}
      }
      */
      ...
      createBoxBody(-5,0,.1,10) // leftwall
      createBoxBody(0,5,10,.1) // top
      createBoxBody(5,0,.1,10) // right wall
      createBoxBody(0,-5,10,.1)
     // createBoxBody(5,5,1,1)

    //  createBoxBody(2,7,.2,7,100)
     // createBoxBody(8,7,.2,7,-100)

      particleSystem.SetRadius(0.035 * 2); // HACK: increase particle radius
...

    {
        ...
        const body = world.CreateBody(circleBodyDef);
        const shape = new box2d.b2CircleShape();
        shape.m_p.Set(1, 2);
        shape.m_radius = 0.5;
        let f=body.CreateFixture( shape,  0.5 );
        f.userData = {type: "circle"}
        */

        createCircleBody(2,4.5);
        createCircleBody(-2,4.0);
    }
  ...
  function update(){
    ...
    function createCircleBody(x,y){
        ...
        //shape.m_p.Set(1, 2);
        shape.m_radius = 0.5;
        let circle =  new box2d.b2CircleShape(.4);
        bd.type =  box2d.b2Body.b2_dynamicBody;
        bd.position.Set(x,y)
        var body = world.CreateBody(bd);
        let fixtureDef = body.CreateFixture(circle, 1);
