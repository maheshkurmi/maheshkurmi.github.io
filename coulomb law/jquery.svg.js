	...
	_cloneAsSVG: function(node) {
		...
		if (node.nodeType == 1) { // element
			...
			for (var i = 0; i < node.attributes.length; i++) {
				var attr = node.attributes.item(i);
				if (attr.nodeName != 'xmlns' && attr.value) {
					if (attr.prefix == 'xlink') {
						newNode.setAttributeNS($.svg.xlinkNS,
							attr.localName || attr.baseName, attr.value);
					}
					else {
						newNode.setAttribute(this._checkName(attr.nodeName), attr.value);
					}
				}
			}
			...
			if ($.trim(node.value)) {
				newNode = this._svg.ownerDocument.createTextNode(node.value);
			}
		}
		else if (node.nodeType == 4) { // CDATA
			if ($.trim(node.value)) {
				try {
					newNode = this._svg.ownerDocument.createCDATASection(node.value);
				}
				catch (e) {
		...
		var loadSVG = function(data) {
			...
			if (data.documentElement.nodeName != 'svg') {
				...
				reportError(!errors.length ? '???' :
					(messages.length ? messages[0] : errors[0]).firstChild.value);
				return;
			}
	...
	_toSVG: function(node) {
		...
		if (node.nodeType == 3) { // Text
			...
			if (node.attributes) {
				for (var i = 0; i < node.attributes.length; i++) {
					var attr = node.attributes.item(i);
					if (!($.trim(attr.value) == '' || attr.value.match(/^\[object/) ||
							attr.value.match(/^function/))) {
