    ...
    (function (shikhar) {
        class Graphics {
            constructor(canvas) {
                ...
                if (this.h_gradient === undefined) {
                    this.h_gradient = null;
                }
                if (this.v_gradient === undefined) {
                    this.v_gradient = null;
                }
                this.canvas = canvas;
                this.PIXEL_SCALE_FACTOR = 2;
                this.context = canvas.getContext("2d");
            }
            begin(width, height, clear) {
            ...
            drawHGradient(c1, c2, x, y, w, h) {
                this.context.save();
                this.h_gradient = this.context.createLinearGradient(x, y, x, y + h);
                this.h_gradient.addColorStop(0, c1.toString());
                this.h_gradient.addColorStop(1, c2.toString());
                this.context.fillStyle = this.h_gradient;
                this.context.fillRect(x, y, w, h);
                this.context.restore();
            }
            drawVGradient(c1, c2, x, y, w, h) {
                this.context.save();
                this.h_gradient = this.context.createLinearGradient(x, y, x + w, y);
                this.h_gradient.addColorStop(0, c1.toString());
                this.h_gradient.addColorStop(1, c2.toString());
                this.context.fillStyle = this.h_gradient;
                this.context.fillRect(x, y, w, h);
